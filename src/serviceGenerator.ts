...
const resolveTypeName = (typeName: string, namespace?: string) => {
    ...
    if (isGenericType(typeName)) {
        return getGenericType(typeName, namespace);        
    }
...
// 判断是否是泛型
function isGenericType(typeName): boolean {
    return typeName?.includes('`1[[');
}

// 提取泛型
function getGenericType(typeName: string, namespace?: string): string {
    // 提取泛型类型
...
function getRefName(refObject: any, namespace: string): string {
    ...
    return resolveTypeName(typeName, namespace) as string;
}
